...
const ContactForm: React.FC<ContactFormProps> = ({ initialContact, onCancel, initialJobDate }) => {
  ...
  const handlePhotoChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (input.files && input.files[0]) {
      ...
      try {
        ...
        addNotification("There was an error processing the photo.", 'error');
      }
    }
    input.value = '';
  }, [addNotification]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        await handleSaveContact({id: contactId, ...contactData}, newFileObjects.current);
        addNotification(contactId ? 'Contact updated successfully!' : 'Contact created successfully!', 'success');
