    ...
    const handlePrint = async () => {
        ...
        try {
            ...
            if (result) {
                ...
                if (!printWindow) {
                    addNotification("Pop-up blocked. Please allow pop-ups for this site.", 'error');
                }
            } else {
    ...
    const handleNativeShare = async () => {
        ...
        try {
             ...
             if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                 ...
                 addNotification("Sharing files is not supported on this device.", 'error');
             }
        } catch (error: any) {
            console.error("Share failed:", error);
             if (error.name !== 'AbortError') {
                addNotification("Share failed. Please try downloading or emailing.", 'error');
            }
        } finally {
            setIsSaving(false);
        }
