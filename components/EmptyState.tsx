...
const EmptyState: React.FC<EmptyStateProps> = ({ Icon, title, message, actionText, onAction }) => {
  return (
    <div className="h-full flex flex-col justify-center items-center text-center p-8 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
      ...
      {actionText && onAction && (
          <button 
              onClick={onAction} 
              className="mt-6 px-4 py-2 bg-sky-500 text-white font-medium rounded-md hover:bg-sky-600 transition-colors shadow-sm"
          >
              {actionText}
          </button>
      )}
